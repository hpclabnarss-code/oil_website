...
# Creates tables on the DB pointed to by DATABASE_URL
...
async def admin_upload(
    ...
    # Create record – store ZIP bytes directly in DB
    record = SpillRecord(
        ...
        zip_data=zip_bytes,          # ✅ stored here
    )
    db.add(record)
    db.commit()
    db.refresh(record)
...
def admin_delete_spill(spill_id: str, token: str = Depends(get_token_from_header), db: Session = Depends(get_db)):
    ...
    if not record:
        raise HTTPException(404, "Spill not found")
    # No external file to delete – just remove DB record
    db.delete(record)
    db.commit()
